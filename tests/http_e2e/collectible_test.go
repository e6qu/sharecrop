...
func TestBundleRewardRequiresBothComponentsAndPaysBothOnAccept(t *testing.T) {
	...
	if accept.CollectibleID != collectibleID {
		t.Fatalf("collectible payout id = %q, want %q", accept.CollectibleID, collectibleID)
	}
	...
	if repeat.PayoutAmount != 25 || repeat.CollectibleID != collectibleID {
		t.Fatalf("idempotent bundle payout = %d/%q, want 25/%q", repeat.PayoutAmount, repeat.CollectibleID, collectibleID)
	}
	if balance := getBalance(t, server, worker.AccessToken); balance.Amount != 125 {
