-- The webhook scopes (webhooks_read, webhooks_manage) exist in the Go scope
-- enum but were missing from the credential scope CHECK constraints, so
-- minting a credential with either scope failed at INSERT time. Re-create both
-- constraints with the full current scope enum. SQLite skips these ALTER
-- statements (unsupported) and strips inline CHECKs; the domain scope enum
