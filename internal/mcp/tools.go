...
// toolAccess says what a caller's credential must hold to see a tool in
// tools/list and to call it. Almost every tool is gated by exactly one agent
// scope; self-introspection (sharecrop.get_my_budget) is deliberately
// scope-free, because a credential may always read its own work policy.
type toolAccess interface {
	toolAccess()
}

// toolNeedsScope gates a tool behind one agent scope.
type toolNeedsScope struct {
	Value agent.Scope
}

// toolNeedsNoScope marks a tool every authenticated credential may call.
type toolNeedsNoScope struct{}

func (toolNeedsScope) toolAccess() {}

func (toolNeedsNoScope) toolAccess() {}
...
func toolDefinitions() []toolDefinition {
	...
	definitions = append(definitions, budgetToolDefinitions()...)
	return definitions
}
...
func coreToolDefinitions() []toolDefinition {
	return []toolDefinition{
		...
		},
		{
			...
			Access:      toolNeedsScope{Value: agent.ScopeSubmissionsWrite},
		...
		},
		{
			...
			Access:      toolNeedsScope{Value: agent.ScopeSubmissionsRead},
		...
		},
		{
			...
			Access:      toolNeedsScope{Value: agent.ScopeSubmissionsReview},
