...
- PR3: comments / clarifying questions — a discussion thread on a task and on a submission so a worker can ask before submitting and a requester can answer (two-way, not just the one-way `review_note`). New `comments` table + endpoints + MCP tools (`post_comment`, `list_comments`) + UI.
